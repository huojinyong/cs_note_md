...
# 防御蒸馏   
第一次先训练网络，得到“教师模型”，得到训练数据的预测标签（软标签，其实为教师模型的输出向量，如[0.8，0，0.2]表示该数字有 80%的可能性被分类为 0，有 20%的可能性被分类为 2），然后把训练数据的标签用得到的软标签替换，第二次训练网络即得蒸馏模型。  
...
**需要注意常数 c 的选择：**  
选择合适的常数 c 对结果效果很重要。先给定 c一个很小的初始值，然后运行$L_2$攻击，如果失败（没有找到对抗样本），则把 c 翻倍再运行。当成功或者 c 达到上限阈值时**终止搜索**。**（哇！原来这就是`搜索 c` 的意思呀！！`搜索`就这？）**   
...
>  因为只要还能继续搜索，在迭代结束前，有几个$\delta_i$大于$\tau$，那么梯度下降后他们也就会小于$\tau$了。当迭代结束时仍然有几个$\delta_i$大于$\tau$，说明梯度已经无法下降了，搜索结束。    

搜索 c 的方法与在 $L_0$中一样。   

`orange:存疑`**不同模型间的结果是不可比较的**。mine：比如虽然从数量上讲，一个 $L_0$算法在 ImageNet 上必须更改（为了生成对抗样本）的像素数量比 MNIST 大很多，但是从比例上讲前者却要比后者小（因为前者图像像素数更多）。因此你很难说明哪个模型需要更改的像素数更少。   
给定任意的$x$，我们都可以生成 targeted $\ x^{'}$。比如给一个全黑图片，竟然也可以生成目标对抗样本，如下图：   
![](./_image/2021-05-24/2021-05-24-20-21-46@2x.png)   
提高蒸馏温度并不能提高神经网络的鲁棒性，只能提高较弱的攻击的失败概率。    