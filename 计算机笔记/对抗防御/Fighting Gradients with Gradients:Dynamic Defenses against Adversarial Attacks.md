...
# 自适应的防御与攻击  
**自适应的防御（防御性熵defensive entropy-dent）**：如下图，每次对输入样本进行分类后，都要：1、对新**输入**进行新的平滑处理$\Sigma$；2、更新**模型**（分类器）参数$\theta + \Delta$    
...
![](./_image/2021-06-10/2021-06-10-10-39-28@2x.png)  
根据上图，我怀疑仿射变换就是:  
$\vec{y} = \beta \vec{x} + \vec{\gamma}$  
也可能是:  
$\vec{y} = \gamma \vec{x} + \vec{\beta}$  
...
$$x^{'} =  \frac{x - \mu}{\sigma}$$   
例子:
![](./_image/2021-06-10/2021-06-10-10-32-54@2x.png)
分别表示:原图->减去均值->除以标准差
来源 : [原文链接](https://www.cnblogs.com/jiading/p/11575038.html)
